async def partner_to_dict(partner: dict):
    return {
        "id": str(partner.get("_id")),
        "tradingName": partner.get("tradingName"),
        "ownerName": partner.get("ownerName"),
        "document": partner.get("document"),
        "coverageArea": await geojson_to_dict(partner.get("coverageArea")),
        "address": await geojson_to_dict(partner.get("address")),
    }


async def geojson_to_dict(geo):
    return {
...
async def validate_and_format_document(document: str):
